...
先划定本章的边界。静态的图像与文档理解——看一张截图、读一幅图表、解析一份 PDF——已经作为感知工具自然融入了前面各章的 Agent 实践：对今天的多模态大模型来说，这类“一次输入、一次理解”的任务已相对成熟，不需要专门的架构设计。本章聚焦的是另一类问题：**实时性把多模态问题变难**的三个场景——语音对话、GUI 操作、机器人控制。在这些场景中，输入是持续流动的、输出必须在严格的时间预算内给出，架构设计因此发生质变。至于连续视觉流（视频）的实时理解，截至写作时对 Agent 而言仍是开放问题——世界模型和观察接口可以部分缓解逐帧截图的局限，但还不能替代连续、可靠的事实观测；Computer Use 部分和章末思考题会再回到这个话题。还要划清另一条边界：多模态**生成**（生图、生视频）在本书框架中只是普通的工具调用（第五章多媒体生成已涉及），Agent 把它当作一个外部工具来使唤即可，并不涉及本章要解决的实时交互难题，因此不在本章主线之内。
...
5. 在机器人与 Computer Use 中引入世界模型：让 Agent 不只根据当前画面反应，还能预测候选动作的后果，并把预测用于长程规划、流式执行和安全重规划。
...
![图9-1 语音 Agent 串行流水线](fig9-1.svg)
...
![图9-2 延迟瀑布：串行累积总响应时间](fig9-2.svg)
...
![图9-4 端到端多模态语音模型架构对比](fig9-4.svg)
...
![图9-6 Step-Audio R1 MGRD 与 MPS 双脑架构](fig9-6.svg)
...
![图9-7 Computer Use Agent 的感知-思考-行动循环](fig9-7.svg)
...
![图9-8 Computer Use 动作空间](fig9-8.svg)
...
![图9-9 Set-of-Mark 与结构化元素索引（browser-use 实现）](fig9-9.svg)
...
![图9-10 分辨率匹配与双向坐标缩放](fig9-10.svg)
...
### Computer Use 的世界模型：从截图循环到可预测桌面

Computer Use 的“截图 → 思考 → 点击”循环，实际上也可以写成一个离散环境的世界模型问题。桌面状态不只是一张像素图，还包括窗口、焦点、滚动位置、输入框内容、加载状态、权限和网络返回；动作则包括点击、键盘输入、滚动、拖拽和等待。理想的模型应学会：

```text
桌面状态 + click/type/scroll/wait ──> 下一状态的表示
```

这样，Agent 可以在真正点击之前比较几个候选动作的后果，在页面加载期间预测下一步，在弹窗一闪而过时从状态差异中恢复，而不必把每一轮都阻塞在“重新截屏、重新理解整张图”上。世界模型还可以在隔离的虚拟桌面中进行反事实试验：先模拟“点击删除”，如果预测会出现不可逆确认框，就请求用户确认，而不是直接把真实文件删掉。

2026 年 7 月，Induction Labs 公布了 **Photon-1**，把这种思路称为 *imagination model*。它不是在预训练阶段逐像素生成截图，而是把每帧压缩为离散的潜在 token，自回归预测下一状态的潜在表示；公司称其用约 200 万段公开电脑操作视频筛选出 5.75 亿帧（约 18 年、每秒一帧）训练一个稀疏 106B-A5B MoE。预训练不需要动作标签，之后再用少于 3.5 万条 computer-use 轨迹教会模型输出动作格式，并用虚拟机上的在线 RL 优化任务完成。给定一张种子截图和后续动作，Photon-1 能连续“想象”桌面状态；模型另外接了一个图像生成器，只用于把潜表示可视化，并非其推理必需组件。[^ch9-20]

这条路线的关键不是“生成一张更逼真的截图”，而是**先预测完成任务所需的下一状态，再生成把当前状态带到该状态的动作**。例如任务是“在 VS Code 新建 Python 文件并写入 hello world”，模型可以先想象文件树出现新文件、编辑器出现文本，再输出点击和输入；执行后只需检查真实状态是否与预测的关键差异一致。

公司报告称 Photon-1 在内部 Computer Use benchmark 上超过 Gemini 3.1 Flash-Lite，服务成本约低三倍；它还展示了在少量棋类和台球数据微调后，潜表示能迁移到棋局演化与物理模拟。这些数字来自 Induction Labs 的内部评测，尚未经过独立复现，读者应把它看作值得跟踪的工程路线，而不是已经确立的行业排名。

对现有 Computer Use Harness 来说，世界模型更可能以**旁路预测器**而不是“一次性替换主模型”的方式落地：主 VLM 仍负责语义理解和工具调用，轻量预测器在后台缓存候选下一状态；动作执行前由它做风险筛查，执行中根据真实屏幕的差异更新潜状态，差异过大便丢弃缓存并重新规划。它与前面的 AOI（关键帧、按需转写、持久叙述）是互补关系：AOI 改善“观察到什么、何时观察”，世界模型回答“采取这个动作后可能发生什么”。

桌面世界模型也比机器人世界模型多出一类困难：网络、广告、权限、验证码和后台数据库会引入不可从画面推断的随机状态。因此预测到的未来只能当作候选假设；删除文件、提交付款、发送邮件等不可逆动作仍要在真实环境中重新观察、校验权限并保留人工确认。对动画和会议等连续输入，世界模型可以减少静态截图的盲区，但不能凭想象补回从未捕获的音频或帧，观察接口仍是第一道事实来源。
...
下面先剖析 VLA 这条主流路线为什么会遇到泛化瓶颈，再看长程规划如何组织任务、如何流式执行，最后引入能够在动作前预测后果的世界模型。这样可以把“规划—执行—预测”三层职责区分开，而不是把所有具身能力都笼统地归为一个端到端模型。

### VLA 的典型问题：语言会说，物理不会想

当前 VLA 的常见配方，是把一个已经会读写文字、理解图像的 VLM 接上动作输出：视觉编码器产生图像 patch，projector 把它们映射到语言模型的输入空间，语言骨干自回归地产生离散动作 token，或由额外的连续动作头生成轨迹。这条路线很实用，但必须看清它的能力来源和边界。

先澄清一个容易被过度简化的说法：**OpenVLA 原始训练并不是“只微调 projector”**。OpenVLA 是在 Prismatic-7B（DINOv2 + SigLIP 视觉编码器、Llama 2 7B）上，用约 97 万条 Open X-Embodiment 机器人轨迹训练的；论文报告了全参数微调，也报告了 LoRA、冻结视觉编码器、只训练最后一层等参数高效方案。Projector 确实是一个很小的两层 MLP，但它不是 OpenVLA 唯一被更新的部分。更准确的表述是：在大多数 VLA 方案中，海量互联网预训练得到的“语言/图像能力”和数量小得多的机器人数据之间，存在一个很窄的适配通道；到了新机器人或新任务的低成本适配阶段，新增能力往往又被压进 projector、LoRA 或动作头等少量参数里。[^ch9-15]

这造成至少四个结构性问题：

1. **数据不平衡**。文本和图像预训练覆盖了数万亿级的语言与静态视觉模式，机器人轨迹却只有几十万到百万级，且集中在少数实验室、物体、相机和机械臂上。模型见过“杯子”这个词，不等于见过不同材质、摩擦系数、装液量下杯子被碰倒的后果。
2. **目标函数不对齐**。行为克隆主要学习 `观测 + 指令 → 下一步动作（或动作块）`，奖励来自示范者是否做得像；它没有被明确要求回答“如果向左推 2 厘米，未来三秒的接触关系会怎样”，也很少从失败中学习反事实后果。动作 token 看似统一了语言和控制，实际上并没有自动产生因果的物理状态。
3. **动作空间与具身绑定**。不同机器人有不同自由度、坐标系、夹爪和控制频率。把它们强行离散成共同 token，或在末端再接一个 embodiment-specific head，都可能让模型学到“这台机器在这类画面下的统计动作”，而不是可迁移的动力学规律。
4. **动作分块放大陈旧状态**。一旦预生成的 0.5–1 秒动作块开始执行，策略在块内看不到新观察；物体被移走、有人伸手、杯子倾倒时，模型仍可能把上一帧的自信当成现实。

所以 OpenVLA 等模型的局限，不是“语言模型完全没有知识”，而是**文本知识、视觉语义和可验证的物理交互知识没有在同一个可行动的状态空间里对齐**。这也解释了为什么增加一个更大的文本 backbone 往往只能改善指令理解，却不能等比例改善接触动力学、长程因果和分布外恢复能力。世界模型要补的，正是“做动作之前先预测动作后果”这一层。
...
**一个具体例子：整理桌面。** 用户说“把桌面整理干净”时，规划器不应直接把这句含糊目标交给 VLA。它先从当前观察建立对象与状态清单，例如：右前方有半杯水、杯口朝上；中央有两张废纸；左侧有三本书；笔记本电脑处于打开状态；桌面有一小块空白区域；垃圾桶和收纳盒分别在桌下与书架旁。接着，它按“先处理风险，再腾出空间，最后清洁”的依赖关系，向执行层下发一组自然语言命令：

1. “移动到桌前，在距离桌沿 30 厘米处停下。”
2. “抓取中央的两张废纸，放进桌下垃圾桶；确认桌面不再有纸团。”
3. “握住右前方杯子的杯身，将它保持竖直并放到右侧托盘；如果检测到液体晃动，降低移动速度。”
4. “合上笔记本电脑，将它移到桌面左后方；不要拉扯仍连接的电源线。”
5. “把三本书按大小叠放在电脑旁，并将散落的笔放入收纳盒。”
6. “确认桌面没有易碎物和通电设备后，擦拭空出的桌面。”
7. “后退一步重新观察，核对桌面是否仍有垃圾、液体或未归位物品。”

这些命令不是一次性的作文，而是带**前置条件、完成条件和风险约束**的子目标。比如第 6 步的前置条件是“杯子与通电设备已移开”，完成条件是“可见污渍消失”，风险约束是“湿布不能接触插座”。VLA 可以自行决定手腕轨迹和夹爪力度，但不能擅自跳过这些高层约束。

**动态环境中的重新规划。** 长程计划只是一份基于时刻 $t$ 的假设；只要用户意图或世界状态变了，未执行的后缀就可能失效。系统因此要保存四类状态：原始目标、最新世界快照、已完成的命令集合、当前正在执行及尚未执行的命令。每个动作块结束都是一个安全点，规划器在这里消费新事件并核对前置条件。处理流程可以归纳为：

1. **判断事件优先级**。用户说“先把电脑收起来”是目标优先级变化；检测到杯子倒了是安全事件，优先级更高。前者通常等当前抓取安全放下后切换，后者立即停止手臂、撤离液体附近并丢弃剩余动作块。
2. **把事实写回世界状态**。不能只在自然语言计划尾部追加一句“杯子倒了”，而要把 `cup.orientation=fallen`、`spill.region=right-front`、`laptop.at_risk=true` 等事实更新为当前真值，并记录哪些旧前置条件已经不成立。
3. **保留已验证的执行前缀，作废受影响的后缀**。已经丢进垃圾桶的废纸不需要再捡；“把竖直的杯子移到托盘”却必须作废，因为它的前置条件已失效。任何依赖“桌面干燥”的擦拭或放置动作也一起失效。
4. **从当前状态生成修订计划**。杯子倾倒后，新的安全序列可能变为：“停止并抬高手臂 → 若水正流向电脑，先断开或移走电脑 → 扶正杯子并放入托盘 → 用吸水布围住液体边缘再向内吸干 → 重新观察 → 恢复未完成的收纳任务。”如果无法判断液体是否危险、插头是否可安全触碰，则停机并请求人类接管。
5. **执行一步、验证一步**。每条新命令完成后都用真实传感器观察与预测状态比较；发现偏差便再次进入上述循环，而不是坚持把旧计划跑完。

这一机制与第四章的异步事件循环一脉相承：普通补充指令可以排队到下一个安全点，碰撞、液体靠近电器、用户喊“停”等紧急事件则主动取消当前动作，提前制造安全点。关键不是每次都“从头重想”，而是**保留已经证实的事实，只重算被变化影响的计划后缀**。

### 流式执行：边规划，边下发，边验证

长程任务也不必等规划器把七八十步全部写完才启动。与第四章介绍的流式工具执行相同，规划器可以在确认一个**可安全执行的前缀**后，把完整命令作为增量事件交给执行器；执行器验证通过便立即开工，规划器同时继续生成后续命令。以“整理桌面”为例：

- 规划器先输出“移动到桌前并停在安全距离”，执行器随即导航；
- 机器人移动期间，规划器继续分析桌面，输出“先清理废纸”和“再移动水杯”；
- 执行器清理废纸时，新观察发现杯子倒了，事件循环撤销尚未开始的“移动水杯”及其后续依赖，规划器从新状态流式补发“保护电脑、处理液体”等命令；
- 只有等吸水和复查通过，先前被暂停的书本、线缆和擦拭任务才重新进入可执行队列。

流式通道传递的是可审计的 command 事件，而不是一段混在思考文本里的指令。例如：

```json
{"type":"command.commit","seq":12,"command_id":"desk-02","command":"抓取中央的两张废纸并放入垃圾桶","preconditions":["paper.visible","bin.reachable"],"success":"paper_count=0","cancel_at":"before_grasp"}
```

执行器回传 `started`、`succeeded`、`cancelled` 或 `failed` 事件；规划器据此更新状态，后续 command 通过 `depends_on` 指向已确认的前置结果。这样既能在规划尚未结束时开工，也不会把“模型说过”误当成“机器人已经做过”。

这里的“流式”不是把一句未生成完的话直接送进电机。**只有语法完整、参数齐全、通过权限与安全校验的一条 command 才能提交执行**；半截 JSON、尚未闭合的自然语言或模型的思考 token 都不能执行。工程上，每条命令至少要带 `command_id`、依赖项、前置条件、成功判据、最大执行时间、可取消点和必要时的补偿动作。规划器还需要接受背压：执行队列已满、环境变化太快或命令依赖尚未满足时，暂停继续下发，避免提前堆出一长串很快就会过时的动作。

因此，流式执行缩短的是**首个安全动作的等待时间**，并把规划计算与物理执行重叠起来；它不等于盲目抢跑。可逆的导航、观察、清理动作可以较早提交，涉及锋利物、通电设备、开门离开安全区等不可逆或高风险动作，则应等更多计划上下文、世界模型预测和安全审批完成后再提交。
...
而**世界模型（world model）**学习的是一个可供推演的环境状态及其转移规律：

```text
当前状态 + 候选动作 ──> 预测的下一状态/未来片段 ──> 选择更可能达成目标的动作
```

用一个简化的形式表示，编码器把历史观察压缩成状态 $z_t=E(o_{\leq t})$，动力学模型预测
$\hat z_{t+1}=F(z_t,a_t)$，规划器在多个候选动作上展开短期 rollout，再由目标函数、安全约束或价值模型选择动作。它至少要回答三个不同问题：**理解**（现在发生了什么）、**预测**（采取某个动作后会发生什么）、**规划**（哪串动作最可能达到目标）。只有把后两项也做出来，模型才称得上可用于行动的世界模型；只会描述视频内容的 VLM 或只会生成好看视频的模型，都不能自动充当可靠的世界模型。

世界模型不是 V-JEPA 的同义词。V-JEPA 属于“在潜表示中预测未来”的一条路线，此外至少还有以下几类：

| 路线 | 主要学习对象 | 典型用途 | 代表工作 |
| --- | --- | --- | --- |
| 表征预测模型 | 不一定解码成像素的未来状态表示 | 物理理解、短视域预测、模型预测控制（MPC） | V-JEPA 2 |
| 生成式交互模型 | 受文本或动作控制的未来视频/环境 | 生成训练环境、可视化反事实、交互式模拟 | Genie 3、Cosmos |
| World-Action Model（WAM） | 动作与未来观察联合建模，常带动作头 | 在策略选择前评估动作后果，辅助机器人控制 | GeniWorld、Robust-WAM 等 |
| 潜在动作/观察学习 | 从无动作标签视频中学习“发生了什么动作”的潜变量 | 利用人类视频扩大训练数据，再迁移到机器人 | LAWM-3D、Photon-1 |
| 模型式强化学习 | 状态转移、奖励/价值和策略 | 在想象轨迹中做规划与 RL，减少真机试错 | Dreamer、MuZero 等 |

不同路线可以组合：一个 V-JEPA 式编码器可以给 WAM 提供稳定状态，一个生成式模型可以作为可视化或仿真器，一个价值模型则负责在 rollout 中选动作。反过来，生成一段视觉上逼真的视频也不代表它的接触动力学正确；“视频生成器”和“可控制的世界模型”不能混为一谈。Genie 3 已展示文本生成、实时交互环境的方向，Cosmos 则把世界基础模型定位为 Physical AI 的可定制数字孪生，两者代表的是生成式世界模型而不是 V-JEPA 的替代品。[^ch9-21]

**它为什么能补上 OpenVLA 的缺口？**

1. **把机器人数据从“动作标签”推广为“观察数据”。** 大量人类视频、机器人视频和屏幕录制都能用于预测未来状态；只有在要把预测与具体执行器对齐时，才需要动作标注或少量交互数据。Meta 的 V-JEPA 2 先用超过 100 万小时视频做无动作预训练，再用约 62 小时机器人数据做动作条件化训练，并在每个时间步用 MPC 重新规划。[^ch9-16]
2. **显式学习动作后果。** 对“抓杯子”“推动杯子”“不动作”分别 rollout，模型必须让三种未来有所区别；这比只模仿示范者的下一 token 更接近因果的物理知识。候选未来还可以在真正执行前被检查碰撞、遮挡、液体泄漏和目标距离。
3. **把策略与具身分开。** 世界模型可以学习“物体如何移动、接触如何改变场景”这类跨平台动力学，再由每台机器人的小型动作头把它翻译成自己的关节或末端控制。2026 年的 DyPES-VLA 就采用“共享动力学先验 + embodiment-specific MoE 动作头”，目标正是避免把不同机器人的动作硬对齐成一套 token。[^ch9-17]
4. **支持闭环重规划与安全拒绝。** 真实观察一旦和预测偏离，模型可以提高不确定度、缩短动作块并重新规划，而不是继续执行过时策略。这与前面的“杯子倒了”例子直接对应：世界模型先预测液体会不会流向电脑，再决定是扶杯、移电脑还是请求人类介入。
5. **允许在想象中训练和评估。** GeniWorld（2026 年 8 月预印本）把数值动作渲染为有空间锚定的视觉动作，并将机器人运动学与环境动力学解耦，尝试在未见过的场景中做闭环 rollout 和策略评估。LAWM-3D 则从多视角人类视频学习 3D 感知的潜在动作，绕开为每个平台手工标注动作空间的成本。它们共同指向一条路线：先从更大、更便宜的观察数据学“世界怎么变”，再用少量机器人数据学“这台机器怎样施加影响”。[^ch9-18]

截至 2026 年 8 月，研究重点已经从“能不能生成未来帧”推进到**未来是否真正受动作控制、是否能跨具身泛化、以及能否实时部署**。Robust-WAM 指出，直接复用为像素重建优化的 VAE 潜空间，容易学到光照和外观捷径；它在动作流中加入语义未来对齐，减少视觉分布变化造成的失效。另一项关于实时 WAM 部署的实证研究比较了同步执行、动作融合和前缀条件生成，发现时间对齐是平滑控制的前提，前缀条件生成在延迟、精度和轨迹连续性之间取得了更好的折中。它们都是预印本结果，不能当作已经解决泛化问题的定论，但说明“预测未来 + 流式控制”正在成为独立于 OpenVLA 的研究主线。[^ch9-19]

世界模型也有明确边界：长时域 rollout 会累积误差；视觉上合理的未来可能违反摩擦、接触或守恒规律；隐藏状态（例如杯中液体量、网页后台状态）可能无法从单帧恢复；不同机器人的执行器延迟也会让同一预测失效。因此，生产系统仍应使用短视域 MPC、动作分块、实时观测、置信度/不确定度估计和硬件安全控制器。世界模型的价值不是替机器人“幻想一个确定答案”，而是为每次行动提供**可比较的后果假设**，并在假设被现实推翻时更快地收缩计划、停止或重规划。

[^ch9-15]: Moo Jin Kim et al. *OpenVLA: An Open-Source Vision-Language-Action Model.* arXiv:2406.09246, 2024. https://arxiv.org/abs/2406.09246 。原论文的全参数微调、冻结视觉编码器、只训练最后一层、Sandwich 和 LoRA 对照见论文第 5.3 节；模型结构与 970k 轨迹说明见 https://openvla.github.io/ 。
...
[^ch9-17]: Junfeng Li et al. *DyPES-VLA: Learning Shared Dynamics Priors and Embodiment-Specific Control for Cross-Embodiment Manipulation.* arXiv:2608.06374, 2026-08-06. https://arxiv.org/abs/2608.06374 。截至本章截稿日为预印本，文中结果应视作研究报告而非独立复现后的定论。
[^ch9-18]: Chenghao Gu et al. *GeniWorld: A Generalizable Interactive World Model for Robotic Manipulation via Visual Actions.* arXiv:2608.06332, 2026-08-06. https://arxiv.org/abs/2608.06332；Jiarui Yang et al. *LAWM-3D: Learning 3D-Aware Latent Actions from Human Videos for Generalizable Robot World Models.* arXiv:2608.05706, 2026-08-06. https://arxiv.org/abs/2608.05706 。两者均为预印本。
[^ch9-19]: Haodong Yan et al. *Robust-WAM: Bridging Generative Pretraining and Semantic Foresight in World-Action Models.* arXiv:2608.05903, 2026-08-06. https://arxiv.org/abs/2608.05903；Motubrain Team, *World Action Models in Real Time: An Empirical Study of Smooth Execution via Asynchronous Deployment.* arXiv:2608.01880, 2026-08-03. https://arxiv.org/abs/2608.01880 。两者均为预印本。
...
![图9-11 VLA 架构（Vision-Language-Action）](fig9-11.svg)
...
![图9-12 Sim2Real 鸿沟与 Domain Randomization](fig9-12.svg)
...
>
>
> ![图9-13 实验 9-10 零样本 RGB Sim2Real 流水线](fig9-13.svg)
>

## 本章小结

三个场景表面差异悬殊，但延迟和多模态这两道坎始终如影随形。语音已走出一条从串行流水线到端到端和全双工、从分离的快慢思考到“边想边说”的演进路径；Computer Use 在 OSWorld 等基准上的准确率已接近人类水平，但操作步骤明显多于人类、步骤耗时随任务推进不断增长的效率差距还没有系统性的解法，世界模型开始提供“预测下一状态、边操作边想”的新方向；机器人则需要把长程规划、流式执行、VLA 控制和世界模型组合起来，才能应对用户指令与物理状态的持续变化。VLA 的文本骨干和少量适配参数带来了很好的起点，却不能替代从视频和交互中学习可行动的世界模型；触觉、灵巧手和长时域物理预测仍是尚未攻克的短板。下一章会把视角拉到多个 Agent 之间的协作，那是另一个维度的挑战。
